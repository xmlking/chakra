...
- **Database GUI**: Run `bun --filter @workspace/db studio` to explore your database
...
bun --filter @workspace/db push

# (Optional) Seed with sample data
bun --filter @workspace/db seed
```

<Callout title="Database GUI" type="idea">

Want to explore your data visually? Run `bun --filter @workspace/db studio` to open Drizzle Studio in your browser.
...
<Callout title="Type Checking" type="info">
  TypeScript checking happens automatically in your editor. For CI/CD, run `bun --filter
  @workspace/api build` to verify types.
</Callout>

### 💡 Hot Tips for Development

- **API Types**: After modifying tRPC routes, types auto-generate — no manual sync needed
- **Database Changes**: Edit `db/schema/`, then run `bun --filter @workspace/db generate` and `push`
...
<Callout title="Build Errors" type="info">
  If TypeScript complains, try `bun --filter @workspace/api build` first. This regenerates type
  definitions.
</Callout>
