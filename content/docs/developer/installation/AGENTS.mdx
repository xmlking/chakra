...
- Make sure you run `turbo run web#dev` and check the app console to see if there are any errors before starting to work on anothers task. fix it first.
...
- Run `turbo run web#build` to verify compilation before major changes
...
- Shadcn/ui for UI components, Lucide icons, cn for classnames
- Payment integration with Creem.io

## Architecture

- Turborepo monorepo: `apps/` and `packages/`
- `@workspace/ui` - shadcn components/hooks, `@workspace/utils` - types/utils, `@workspace/db` - drizzle schema and db, `@workspace/email` for email utils and templates
...
- Use `cn` for conditional class names and merging Tailwind classes
- Use `motion/react` for animations

## HTTP Client & API Requests

- **Always use the centralized ky HTTP client**: `import { http } from '@workspace/shared/lib/http-client'`
...
- Use Pino logger from `@workspace/shared/logger` for structured logging with automatic PII redaction
- Use `toast` from `@workspace/ui` for user notifications
- Use `ErrorBoundary` for catching errors in React components

## Logging

- DO NOT USE console.log/error/warn\*\* - Always use Pino logger log.info/error/debug/warn (`@workspace/shared/lib/logger`)
- **Import**: `import { log } from '@workspace/shared/lib/logger'` for basic logging (use `log` not `logger`)
